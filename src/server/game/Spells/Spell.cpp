...
void Spell::SearchChainTargets(std::list<WorldObject*>& targets, uint32 chainTargets, WorldObject* target, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectType, SpellTargetSelectionCategories  /*selectCategory*/, ConditionList* condList, bool isChainHeal)
{
    ...
    switch (m_spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
            ...
            if (isChainHeal)
                jumpRadius = 15.0f; //molly: changed to 15 yards
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                // Set aura charge amount to desired value
                if (m_spellValue->AuraChargeAmount > 0)
                {
                    if (!refresh)
                        m_spellAura->SetCharges(m_spellValue->AuraChargeAmount);
                    else
                        m_spellAura->ModCharges(m_spellValue->AuraChargeAmount);
                }
...
void Spell::SendSpellGo()
{
    ...
    if (((IsTriggered() && !m_spellInfo->IsAutoRepeatRangedSpell()) || m_triggeredByAuraSpell)
        && !m_cast_count && !(m_spellInfo->IsChanneled()
            || (m_spellInfo->CastTimeEntry && m_spellInfo->CastTimeEntry->CastTime > 0)))
        castFlags |= CAST_FLAG_PENDING;
...
void Spell::SetSpellValue(SpellValueMod mod, int32 value)
{
    switch (mod)
    {
        ...
        case SPELLVALUE_AURA_STACK:
            ...
            break;
        case SPELLVALUE_AURA_CHARGE:
            m_spellValue->AuraChargeAmount = uint8(value);
            break;
